...
resource "aws_route53_record" "argocd" {
  # count = var.ingress_config.enable && var.ingress_config.auto_create_route53_record && var.ingress_config.route53_zone_name != "" && try(length(data.kubernetes_ingress_v1.argocd[0].status[0].load_balancer[0].ingress) > 0, false) ? 1 : 0
count = var.ingress_config.enable && var.ingress_config.auto_create_route53_record && var.ingress_config.route53_zone_name != "" ? 1 : 0
  ...
  alias {
    name                   = data.kubernetes_ingress_v1.argocd[0].status[0].load_balancer[0].ingress[0].hostname
